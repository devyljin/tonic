...
gum confirm "Voulez vous Télécharger des dump ?" --affirmative "Oui !" --negative "Non." && DOWNLOAD_CHOICE=1 || DOWNLOAD_CHOICE=0
...
gum confirm "Voulez importer des dump ?" --affirmative "Oui !" --negative "Non." && IMPORT_CHOICE=1 || IMPORT_CHOICE=0
...
if [[ "$DOWNLOAD_CHOICE" == 0 && "$IMPORT_CHOICE" == 0 ]]; then
  gum style --border normal --margin "1" --padding "1 2" --border-foreground 212 "Pourquoi m'avez vous appelé ? Y'en a qui essaye de se reposer !"
fi
